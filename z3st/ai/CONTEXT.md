...
- `verification/mechanics/elliptical_cavity_2D` — elastic, `oxide.yaml`, no damage. It is
...
## 10.2 Work in progress: phase-field fracture (UO2)

Two threads, both phase-field, on calibrations that must not be mixed:

1. Cavity / bubble micromechanics — the case family in §15b, on `uo2_jiang.yaml` at
   sigma_c = 150 MPa, isothermal. Status, decisions and open questions are in
   `benchmarks/damage/CALIBRATION.md`; read that file, not this section, before touching
   any of those cases.
   Next step: an l-convergence check. Three of the cases sit exactly at `l = 4h` with no
   margin, and three headline numbers moved once discretisation was fixed — the two-cavity
   strength halved when its mesh floor was corrected (116.52 -> 53.65 MPa), and
