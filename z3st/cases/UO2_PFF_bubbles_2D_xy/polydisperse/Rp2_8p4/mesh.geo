...
EndIf
If(!Exists(Rp_2))
  Rp_2 = 8.4e-6;       // bubble B projected radius (m), centered on xmax -- ratio Rp_1/Rp_2=1.33 consolidation run
EndIf
If(!Exists(theta_deg))
  theta_deg = 48.1;    // semi-dihedral angle (deg), same for both bubbles
EndIf
If(!Exists(Lx))
  Lx = 42.2e-6;        // center(A)-to-center(B) spacing (m); gives a
                        // 22.6e-6 ligament (same as the Rp_2=5.6um run),
                        // matching periodic_row/Lx_45 (monodisperse) too
EndIf
If(!Exists(Ly))
  Ly = 60e-6;
EndIf
