...
EndIf
If(!Exists(theta_deg))
  theta_deg = 60.0;    // Semi-dihedral angle (deg)
EndIf
If(!Exists(Lx))
  Lx = 60e-6;
EndIf
...
EndIf
If(!Exists(h_cavity))
  h_cavity = 2.488889e-07;   // fine mesh size at the cavity boundary (chosen well below lc/4 = 0.5e-6; NOT a curvature-resolution criterion -- the tip is a wedge corner, not an ellipse, see README correction note)
EndIf
If(!Exists(h_plate))
